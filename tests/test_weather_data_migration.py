...
class TestWeatherDataTableStructure:
    ...
    def test_required_columns_exist(self, cursor):
        ...
        weather_fields = [
            ...
            "weather_code", "cloud_cover", "visibility", "pressure_sea_level",
            "pressure_surface_level", "dew_point", "uv_index"
        ]
...
class TestDataInsertion:
    ...
    def test_insert_full_weather_data(self, db_conn, cursor, sample_location_id):
        ...
        """, (
            ...
            'hourly',
            # Weather data
            25.5, 28.0, 5.2, 8.5,  # temp, apparent_temp, wind_speed, wind_gust
            180, 65.0, 20.0,       # wind_direction, humidity, precip_prob
            1000, 25.0, 16.0,      # weather_code, cloud_cover, visibility
            1015.5, 1013.2, 18.5, 2  # pressures, dew_point, uv_index
        ))
        db_conn.commit()
    ...
    def test_reject_invalid_granularity(self, db_conn, cursor, sample_location_id):
        ...
        with pytest.raises((pg_errors.CheckViolation, pg_errors.StringDataRightTruncation)):
            cursor.execute("""
                INSERT INTO weather_data (
    ...
    def test_composite_pk_prevents_duplicates(self, db_conn, cursor, sample_location_id):
    ...
    def test_different_granularity_same_timestamp_allowed(self, db_conn, cursor, sample_location_id):
    ...
    def test_latest_temperature_query(self, db_conn, cursor, sample_location_id):
        ...
        cursor.execute("""
            INSERT INTO weather_data (
                ...
                (%s, %s, 'hourly', 24.0, 4.0);
        """, (
            sample_location_id, datetime(2024, 2, 1, 10, 0, 0, tzinfo=timezone.utc),
            sample_location_id, datetime(2024, 2, 1, 11, 0, 0, tzinfo=timezone.utc),
            sample_location_id, datetime(2024, 2, 1, 12, 0, 0, tzinfo=timezone.utc),
        ))
        db_conn.commit()
...
class TestForeignKeyBehavior:
    ...
    def test_delete_cascade(self, db_conn, cursor):
        ...
        # Verify data exists
        cursor.execute("SELECT COUNT(*) as count FROM weather_data WHERE location_id = %s;", (temp_location_id,))
