...
class TestSettingsValidation:
    ...
    def test_required_api_key(self):
        ...
        assert "TOMORROW_API_KEY" in str(exc_info.value) or "cannot be empty" in str(
            exc_info.value
        )
    ...
    def test_required_pg_password(self):
        ...
        assert "PGPASSWORD" in str(exc_info.value) or "cannot be empty" in str(
            exc_info.value
        )
    ...
    def test_valid_settings(self):
        ...
        settings = Settings(
            tomorrow_api_key="test_api_key_123", pg_password="test_password"
        )
...
class TestValidationConstraints:
    ...
    def test_fetch_interval_range(self):
        ...
        with pytest.raises(ValidationError):
            Settings(
                tomorrow_api_key="test", pg_password="test", fetch_interval_minutes=1
            )

        with pytest.raises(ValidationError):
            Settings(
                tomorrow_api_key="test", pg_password="test", fetch_interval_minutes=2000
            )

    def test_log_level_valid_values(self):
        """Should only accept valid log levels."""
        settings = Settings(
            tomorrow_api_key="test", pg_password="test", log_level="DEBUG"
        )
        assert settings.log_level == "DEBUG"

        settings = Settings(
            tomorrow_api_key="test", pg_password="test", log_level="ERROR"
        )
...
class TestComputedProperties:
    ...
    def test_is_production(self):
        ...
        dev_settings = Settings(
            tomorrow_api_key="test", pg_password="test", environment="development"
        )
        assert dev_settings.is_production is False

        prod_settings = Settings(
            tomorrow_api_key="test", pg_password="test", environment="production"
        )
...
class TestEnvironmentVariables:
    ...
    @patch.dict(
        os.environ,
        {"TOMORROW_API_KEY": "test_key", "PGPASSWORD": "test_pass"},
        clear=True,
    )
...
class TestSettingsCaching:
    ...
    @patch.dict(
        os.environ,
        {"TOMORROW_API_KEY": "cached_key", "PGPASSWORD": "cached_pass"},
        clear=True,
    )
    ...
    @patch.dict(
        os.environ,
        {"TOMORROW_API_KEY": "original_key", "PGPASSWORD": "original_pass"},
        clear=True,
    )
    def test_reload_settings(self):
        ...
        # Change environment
        with patch.dict(
            os.environ, {"TOMORROW_API_KEY": "new_key", "PGPASSWORD": "new_pass"}
        ):
            reload_settings()
            settings2 = get_settings()
