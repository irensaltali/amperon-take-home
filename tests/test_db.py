...
def sample_location():
    ...
    with get_cursor() as cleanup_cursor:
        cleanup_cursor.execute(
            "DELETE FROM weather_data WHERE location_id = %s", (loc.id,)
        )
...
class TestConnectionPool:
    ...
    def test_connection_rollback_on_error(self):
        ...
        with get_cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) as count FROM locations WHERE lat = 0 AND lon = 0"
            )
            result = cur.fetchone()
...
class TestLocationOperations:
    ...
    def test_get_location_by_coordinates_found(self, sample_location):
        ...
        location = get_location_by_coordinates(sample_location.lat, sample_location.lon)
...
class TestWeatherDataOperations:
    ...
    def test_insert_readings_upsert(self, sample_location):
        ...
        # Verify update
        readings = get_time_series(sample_location.id, timestamp, timestamp, "hourly")
        assert len(readings) == 1
    ...
    def test_get_time_series(self, sample_location, sample_weather_readings):
        ...
        readings = get_time_series(sample_location.id, start_time, end_time, "hourly")
